...
public class Yachiru : Minion
{
    ...
    public void Skill()
    {
        ...
        if (faceRight)
        {
            skillClone = Instantiate(skill, new Vector3(transform.position.x - 0.5f, transform.position.y), Quaternion.Euler(new Vector3(0, 0, 0)));
            skillClone.GetComponent<YachiruSkill>().Initialize(Vector2.left);
            skillClone.GetComponent<YachiruSkill>().Flip();
        }
        else
        {
            skillClone = Instantiate(skill, new Vector3(transform.position.x + 0.5f, transform.position.y), Quaternion.Euler(new Vector3(0, 0, 180)));
            skillClone.GetComponent<YachiruSkill>().Initialize(Vector2.right);
            skillClone.GetComponent<YachiruSkill>().Flip();
        }
    }
    ...
    public void AlertObservers(string message)
    {
        switch (message)
        {
            case "AttackEnd":
                ...
                break;
            case "Skill":
                var newPos = new Vector2(transform.position.x + speed*10f, transform.position.y);
